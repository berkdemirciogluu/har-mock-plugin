import { inject, DestroyRef, type Type } from '@angular/core';
...
function clearGuardsRecursively(routes: Route[], preserve: PreserveList): void {
  for (const route of routes) {
    route.canActivate = filterGuards(
      route.canActivate as unknown[],
      preserve,
    ) as typeof route.canActivate;
    route.canDeactivate = filterGuards(
      route.canDeactivate as unknown[],
      preserve,
    ) as typeof route.canDeactivate;
...
export function harMockGuardBypassFactory(): () => void {
  ...
  return () => {
    // Devre dışıysa veya bypassGuards=false ise hiçbir şey yapma (ARCH10)
    if (!config.enabled || !config.bypassGuards) return;

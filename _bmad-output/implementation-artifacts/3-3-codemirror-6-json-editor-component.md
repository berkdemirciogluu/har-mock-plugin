...
- [x] Task 1: CodeMirror 6 paket kurulumu (AC: #1, #4)
  - [x] Subtask 1.1: `packages/extension/package.json`'a bağımlılık ekle: `@codemirror/view`, `@codemirror/state`, `@codemirror/lang-json`, `@codemirror/lint`, `@codemirror/commands`
  - [x] Subtask 1.2: `yarn install` ile paketleri kur

- [x] Task 2: `hm-json-editor` component oluşturma (AC: #1, #2, #5, #7)
  - [x] Subtask 2.1: Dosyaları oluştur:
  ...
  - [x] Subtask 2.2: Signal-based inputs tanımla: `value = input<string>('')` ve `readonly = input<boolean>(false)`
  - [x] Subtask 2.3: `valueChange = output<string>()` output tanımla
  - [x] Subtask 2.4: `viewChild<ElementRef>('editorContainer')` ile container referansı al
  - [x] Subtask 2.5: `DestroyRef` inject edip `onDestroy` callback'inde `editorView.destroy()` çağır

- [x] Task 3: CodeMirror 6 EditorView entegrasyonu (AC: #1, #2, #3, #6)
  - [x] Subtask 3.1: `afterNextRender` ile EditorView'i ilk render'da initialize et (DOM hazır olduğunda) — Not: `afterRenderEffect` Angular 18.2'de mevcut değil; `afterNextRender` kullanıldı
  - [x] Subtask 3.2: EditorState extensions: `json()`, `lintGutter()`, `lintKeymap`, temel keymap'ler, `EditorView.theme()` ile özel stil
  - [x] Subtask 3.3: `readonly = true` ise `EditorState.readOnly.of(true)` + `EditorView.editable.of(false)` ekle
  - [x] Subtask 3.4: `updateListener` extension ile JSON valid olduğunda `valueChange.emit()` tetikle
  - [x] Subtask 3.5: `value` input değiştiğinde `effect()` ile EditorView content'ini programmatic olarak güncelle (`dispatch` + `transaction`)

- [x] Task 4: Read-only mode implementasyonu (AC: #2)
  - [x] Subtask 4.1: `readonly` input'u izle; değiştiğinde EditorView'e `Compartment` reconfigure ile güncelle
  - [x] Subtask 4.2: `readonly: true` iken cursor'ı gizle: `.cm-cursor { display: none }` tema override (`themeCompartment`)

- [x] Task 5: JSON lint/validation (AC: #3)
  - [x] Subtask 5.1: `@codemirror/lint`'ten `linter()` ile JSON parse hatalarını yakala
  - [x] Subtask 5.2: `jsonParseLinter()` kullan (lang-json'da built-in) — manuel linter yazmaya gerek yok
  - [x] Subtask 5.3: `updateListener`'da geçerli JSON kontrolü: `JSON.parse(doc.toString())` try/catch, hata varsa emit etme

- [x] Task 6: Unit testler (AC: #1–7)
  - [x] Subtask 6.1: Component create testi — fixture oluşturulabilmeli, hata yokken `CodeMirrorMock` kullanılabilir
  - [x] Subtask 6.2: `readonly: true` input testi — CSS veya `EditorState.readOnly` attribute kontrol
  - [x] Subtask 6.3: `value` input güncellendiğinde EditorView dispatch tetiklenmeli
  - [x] Subtask 6.4: Geçerli JSON → `valueChange` emit; geçersiz JSON → emit yok
  - [x] Subtask 6.5: `ngOnDestroy` veya DestroyRef callback → `editorView.destroy()` çağrıldığını doğrula
...
- `afterRenderEffect` Angular 18.2'de mevcut değil (`afterNextRender` kullanıldı — story dev notes'ı corrected)
- Jest mock'larında `MockEditorView.mock.instances[0]` yanlış context döndürüyor; `mock.results[0].value` doğru yaklaşım
- CodeMirror ESM paketleri `transformIgnorePatterns`'a eklenmeden `jest.mock()` ile tamamen mock'lanarak çözüldü

### Completion Notes List

- ✅ `hm-json-editor` Angular 18 standalone component oluşturuldu (`templateUrl` ayrı HTML, `OnPush`, `inject()`, signal API)
- ✅ CodeMirror 6 entegrasyonu: `afterNextRender` ile DOM hazır olunca `EditorView` init, `Compartment` ile runtime konfigürasyon
- ✅ Read-only mode: `editableCompartment` + `themeCompartment` ile cursor gizleme
- ✅ JSON lint/validation: `jsonParseLinter()` ile otomatik, `updateListener`'da try/catch ile valid JSON kontrolü
- ✅ 12 unit test — tüm AC'ler kapsandı; 335/335 toplam test geçti (0 regresyon)
- ✅ `@codemirror/commands` bağımlılığı story spec'ine ek olarak eklendi (`defaultKeymap` için)
...
_bmad-output/implementation-artifacts/3-3-codemirror-6-json-editor-component.md

## Change Log

- 2026-02-27: Story 3.3 implement edildi — `hm-json-editor` CodeMirror 6 tabanlı Angular component oluşturuldu. Tüm task/subtask'lar tamamlandı, 12 unit test eklendi, 335/335 test geçti (Claude Sonnet 4.6)
- 2026-02-27: Code review tamamlandı — 9 bulgu düzeldi (2 HIGH, 4 MEDIUM, 3 LOW). Pre-existing Babel hoisting bug (jest.mock factory TypeScript parse hatası) düzeltildi. 335/335 test geçti (Claude Sonnet 4.6)
